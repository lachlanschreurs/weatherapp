    ...
    try {
      ...
      if (data.radar) {
        ...
        if (data.radar.past && data.radar.past.length > 0) {
          ...
          frames.push({
            url: `https://tilecache.rainviewer.com${lastPast.path}/256/{z}/{x}/{y}/4/1_1.png`,
            time: lastPast.time,
            isForecast: false
          });
        }
        ...
        if (data.radar.nowcast) {
          ...
          const forecastFrames = data.radar.nowcast
            .filter((frame: any) => {
              const timeDiff = (frame.time - currentTime) / 60;
              return timeDiff <= 120;
            })
            .map((frame: any) => ({
          ...
          {!isLoading && radarFrames.length > 0 && (
            <div className="bg-gradient-to-r from-gray-50 to-blue-50 px-4 py-3 border-t border-gray-200">
              ...
              <div className="mt-2 text-xs text-gray-500 text-center">
                Showing current conditions + 2-hour forecast
              </div>
            </div>
          )}
        </>
