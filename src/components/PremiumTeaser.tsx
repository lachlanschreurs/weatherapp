import { Lock, FileText, Brain, Activity } from 'lucide-react';
...
export function PremiumTeaser({ onSignUpClick }: PremiumTeaserProps) {
  const features = [
    {
      icon: FileText,
      label: 'Weekly spray reports',
    },
    {
      icon: Brain,
      label: 'Farmer Joe AI advice',
    },
    {
      icon: Activity,
    ...
    <div className="rounded-2xl border border-slate-700/50 bg-slate-900/60 backdrop-blur-sm overflow-hidden shadow-xl">
      ...
      <div className="px-6 py-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
            ...
            return (
              <div key={index} className="flex items-center gap-2.5">
                ...
                {index < features.length - 1 && (
                  <div className="hidden sm:block w-px h-5 bg-slate-700/60 ml-2" />
                )}
              </div>
            );
          })}
        ...
        <div className="flex flex-col items-start sm:items-end gap-2 flex-shrink-0">
          ...
          <span className="text-xs text-slate-500">Unlock premium features with your free 30-day trial.</span>
        </div>
      </div>
    </div>
  );
